use std::{collections::VecDeque, sync::atomic::{AtomicBool, AtomicUsize}, time::Instant};

use crate::rpc::RpcBase;
use grapple_frc_msgs::{MessageId, bridge::BridgedCANMessage, grapple::{GrappleDeviceMessage, GrappleMessageId}};
...
impl Filter {
  pub fn accept<'a>(&self, msg: &BridgedCANMessage<'a>, defrag: &Option<&GrappleDeviceMessage<'a>>) -> bool {
    match self {
...
impl CanLog {
  ...
  pub async fn on_message<'a>(&self, msg: &BridgedCANMessage<'a>, defrag: Option<&GrappleDeviceMessage<'a>>) {
    ...
    if self.logging_enabled.load(std::sync::atomic::Ordering::Relaxed) {
      let item = MailboxItem {
        seq,
        raw: BridgedCANMessage { id: msg.id, timestamp: elapsed, data: msg.data.to_static() },
