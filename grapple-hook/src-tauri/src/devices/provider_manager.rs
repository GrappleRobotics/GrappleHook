...
use super::{
    generic_usb::GenericUSB,
    provider::{
        DeviceProvider, ProviderInfo, WrappedDeviceProvider, WrappedDeviceProviderRequest,
        WrappedDeviceProviderResponse,
    },
    roborio::daemon::RoboRioDaemon,
};
...
impl ProviderManager {
    ...
    pub async fn detect_devices(&self) -> anyhow::Result<()> {
        let mut providers: tokio::sync::RwLockWriteGuard<HashMap<String, ProviderContainer>> =
                            ...
                            if usbi.vid == 0x3580 && usbi.pid == 0x4000 {
                                ...
                                if !providers.contains_key(&addr) {
                                    providers.insert(
                                        addr.clone(),
                                        ProviderContainer {
                                            provider: WrappedDeviceProvider::new(Box::new(
                                                GenericUSB::new(addr),
                                            )),
                                            is_autodetect: true,
                                            last_autodetect: now,
        ...
        providers.retain(|k, v| {
            *is_connected.get(k).unwrap()
                || !v.is_autodetect
                || v.last_autodetect.elapsed().as_secs() < 2
        });
        Ok(())
    }
}
...
impl ProviderManager {
    ...
    ) -> anyhow::Result<WrappedDeviceProviderResponse> {
        self.providers
            .read()
            .await
            .get(&address)
