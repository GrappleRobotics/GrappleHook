...
use grapple_frc_msgs::{
    binmarshal::{AsymmetricCow, MarshalUpdate, Payload},
    grapple::{
        device_info::GrappleModelId,
        errors::GrappleError,
        firmware::{FlashParameters, GrappleFirmwareMessage, UpdatePartV2Payload},
        GrappleDeviceMessage, GrappleMessageId, Request, TaggedGrappleMessage,
    },
    request_factory, MessageId, Validate, DEVICE_ID_BROADCAST,
};
...
impl SendWrapper {
    ...
    async fn request_inner(
        &self,
        msg: TaggedGrappleMessage<'static>,
        reply_id: GrappleMessageId,
        timeout_ms: usize,
    ) -> anyhow::Result<TaggedGrappleMessage> {
    ...
    async fn request(
        &self,
        mut msg: TaggedGrappleMessage<'static>,
        timeout_ms: usize,
        retry: usize,
    ) -> anyhow::Result<TaggedGrappleMessage> {
        ...
        match self
            .request_inner(msg.clone(), complement_id, timeout_ms)
            .await
        {
            Ok(x) => Ok(x),
...
impl DeviceInfo {
    pub fn require_serial(&self) -> anyhow::Result<u32> {
        return self
            .serial
            .ok_or(anyhow::anyhow!("No Serial Number for Device!"));
    }

    pub fn require_device_id(&self) -> anyhow::Result<u8> {
        return self
            .device_id
            .ok_or(anyhow::anyhow!("No Device ID for Device!"));
    }
}

#[async_trait::async_trait]
pub trait Device: RpcBase {
    async fn handle(&self, _msg: TaggedGrappleMessage<'static>) -> anyhow::Result<()> {
        Ok(())
    }
}
...
impl GrappleDevice {
    async fn blink(&self) -> anyhow::Result<()> {
        self.sender
            .send(TaggedGrappleMessage::new(
                DEVICE_ID_BROADCAST,
                grapple_frc_msgs::grapple::GrappleDeviceMessage::Broadcast(
                    grapple_frc_msgs::grapple::GrappleBroadcastMessage::DeviceInfo(
                        grapple_frc_msgs::grapple::device_info::GrappleDeviceInfo::Blink {
    ...
    async fn set_id(&self, id: u8) -> anyhow::Result<()> {
        self.sender
            .send(TaggedGrappleMessage::new(
                ...
                grapple_frc_msgs::grapple::GrappleDeviceMessage::Broadcast(
                    grapple_frc_msgs::grapple::GrappleBroadcastMessage::DeviceInfo(
                        grapple_frc_msgs::grapple::device_info::GrappleDeviceInfo::SetId {
                            serial: self.info.read().await.require_serial()?,
                            new_id: id,
                        },
                    ),
    ...
    async fn set_name(&self, name: String) -> anyhow::Result<()> {
        self.sender
            .send(TaggedGrappleMessage::new(
                ...
                grapple_frc_msgs::grapple::GrappleDeviceMessage::Broadcast(
                    grapple_frc_msgs::grapple::GrappleBroadcastMessage::DeviceInfo(
                        grapple_frc_msgs::grapple::device_info::GrappleDeviceInfo::SetName {
                            serial: self.info.read().await.require_serial()?,
    ...
    async fn commit_to_eeprom(&self) -> anyhow::Result<()> {
        self.sender
            .send(TaggedGrappleMessage::new(
                ...
                grapple_frc_msgs::grapple::GrappleDeviceMessage::Broadcast(
                    grapple_frc_msgs::grapple::GrappleBroadcastMessage::DeviceInfo(
                        grapple_frc_msgs::grapple::device_info::GrappleDeviceInfo::CommitConfig {
                            serial: self.info.read().await.require_serial()?,
                        },
                    ),
                ),
...
impl<T: FirmwareValidatingDevice> FirmwareUpgradeDevice<T> {
    ...
    pub async fn field_upgrade_worker(
        sender: SendWrapper,
        id: u8,
        data: &[u8],
        progress: Arc<RwLock<Option<f64>>>,
        ...
        *progress.write().await = Some(0.0);
        ...
        let (encode, decode) = request_factory!(
            dat2,
            GrappleDeviceMessage::FirmwareUpdate(GrappleFirmwareMessage::GetFlashParameters(dat2))
        );

        let msg = sender
            .request(TaggedGrappleMessage::new(id, encode(())), 300, 5)
            .await;
        ...
        for (i, chunk) in chunks.enumerate() {
            ...
            match flash_params.flash_compat_version {
                    ...
                    sender
                        .send(TaggedGrappleMessage::new(
                            ...
                            GrappleDeviceMessage::FirmwareUpdate(
                                GrappleFirmwareMessage::UpdatePart(
                                    AsymmetricCow(Cow::<Payload>::Borrowed(Into::into(
                                        &padded[..],
                                    )))
                                    .into_static(),
                                ),
                ...
                2.. => {
                    let (part_encode, part_decode) = request_factory!(
                        dat2,
                        GrappleDeviceMessage::FirmwareUpdate(GrappleFirmwareMessage::UpdatePartV2(
                            dat2
                        ))
                    );
                    ...
                    let msg = sender
                        .request(
                            TaggedGrappleMessage::new(
                                id,
                                part_encode(UpdatePartV2Payload {
                                    offset: i as u32 * flash_params.payload_len as u32,
                                    payload: Cow::<Payload>::Borrowed(Into::into(&padded[..]))
                                        .into_static()
                                        .into(),
                                }),
                            ),
        ...
        *progress.write().await = Some(100.0);
        sender
            .send(TaggedGrappleMessage::new(
                id,
                GrappleDeviceMessage::FirmwareUpdate(GrappleFirmwareMessage::UpdateDone),
            ))
            .await?;
...
pub async fn start_field_upgrade(sender: &SendWrapper, serial: u32) -> anyhow::Result<()> {
    sender
        .send(TaggedGrappleMessage::new(
            DEVICE_ID_BROADCAST,
            GrappleDeviceMessage::FirmwareUpdate(GrappleFirmwareMessage::StartFieldUpgrade {
                serial,
            }),
        ))
        .await
...
fn maybe_unpack_firmware(data: &[u8]) -> anyhow::Result<Vec<u8>> {
    ...
    let mut firmware_f = archive.by_name(
        index
            .get("firmware_update_bin")
            .map(|x| x.as_str())
            .flatten()
...
#[rpc]
impl<T: FirmwareValidatingDevice + HasFirmwareUpdateURLDevice + Send + Sync>
    FirmwareUpgradeDevice<T>
{
    async fn do_field_upgrade(&self, data: Vec<u8>) -> anyhow::Result<()> {
        let buf = match maybe_unpack_firmware(&data) {
            ...
            Err(_) => {
                let d = data;
                <T>::validate_firmware(&*self.info.read().await, &d)
                    .map_err(|e| anyhow::anyhow!("Not a valid firmware file: {}", e))?;
                d
            }
        };
        ...
        tokio::task::spawn(async move {
            let d = buf;
            let r =
                Self::field_upgrade_worker(sender, id, &d[..], progress, notify, chunk_size).await;
...
impl<T: FirmwareValidatingDevice + HasFirmwareUpdateURLDevice + Send + Sync> RootDevice
    for FirmwareUpgradeDevice<T>
{
    fn device_class(&self) -> &'static str {
...
#[async_trait::async_trait]
impl<T: FirmwareValidatingDevice + HasFirmwareUpdateURLDevice + Send + Sync> Device
    for FirmwareUpgradeDevice<T>
{
    async fn handle(&self, msg: TaggedGrappleMessage<'static>) -> anyhow::Result<()> {
        if msg.device_id == DEVICE_ID_BROADCAST
            || Some(msg.device_id) == self.info.read().await.device_id
        {
            match msg.clone().msg {
...
#[async_trait::async_trait]
pub trait VersionGatedDevice:
    HasFirmwareUpdateURLDevice + RootDevice + Sized + Sync + 'static
{
    ...
    async fn maybe_gate<F: FnOnce(SendWrapper, Arc<RwLock<DeviceInfo>>) -> Self + Send>(
        send: SendWrapper,
        info: Arc<RwLock<DeviceInfo>>,
        create_fn: F,
    ) -> Box<dyn RootDevice + Send + Sync + 'static> {
        match Self::validate_version(info.clone().read().await.firmware_version.clone()) {
            Ok(_) => Box::new(create_fn(send, info)),
            Err(e) => Box::new(OldVersionDevice::new(
                send,
                info,
                format!("{}", e),
                Self::firmware_url(),
...
async fn check_for_new_firmware_release_rpc_target<T: VersionGatedDevice>(
    info: &SharedInfo,
