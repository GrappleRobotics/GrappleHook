...
use grapple_frc_msgs::{
    grapple::{
        ...
        errors::GrappleError,
        mitocandria::{
            self, MitocandriaAdjustableChannelCalibrationRequest,
            MitocandriaAdjustableChannelRequest, MitocandriaSwitchableChannelRequest,
        },
...
impl VersionGatedDevice for Mitocandria {
    ...
    async fn check_for_new_firmware_release(current_version: &str) -> Option<LightReleaseResponse> {
        ...
        most_recent_update_available(
            ...
            |release| match release.tag_name.to_string() {
                x if x.starts_with("mitocandria") => {
                    if let Some(captures) = regex.captures(x.as_str()) {
                        let vers = semver::Version::parse(
                            captures.get(2).map(|x| x.as_str()).unwrap_or(""),
                        )
                        .ok();
                        println!("{:?}", vers);
...
impl FirmwareValidatingDevice for Mitocandria {
    fn validate_firmware(_info: &super::DeviceInfo, buf: &[u8]) -> anyhow::Result<()> {
        if &buf[0x200..0x204] == &[0xBEu8, 0xBAu8, 0xFEu8, 0xCAu8]
            && buf[0x20c] == (GrappleModelId::MitoCANdria as u8)
        {
            Ok(())
        } else {
...
impl Mitocandria {
    ...
    async fn set_switchable_channel(
        &self,
        ...
        let (encode, decode) = request_factory!(
            data,
            GrappleDeviceMessage::PowerDistributionModule(
        ...
        let msg = self
            .sender
            .request(TaggedGrappleMessage::new(id, encode(channel)), 300, 5)
            .await?;
        decode(msg.msg)??;
        Ok(())
    }

    async fn set_adjustable_channel(
        &self,
    ...
    async fn calibrate_adjustable_channel(&self) -> anyhow::Result<()> {
        ...
        let (encode, decode) = request_factory!(
            ...
            GrappleDeviceMessage::PowerDistributionModule(
                mitocandria::MitocandriaMessage::ChannelRequest(
                    mitocandria::MitocandriaChannelRequest::StartAutoCalibrate(data)
                )
            )
        );
