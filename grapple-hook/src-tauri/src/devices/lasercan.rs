use grapple_frc_msgs::{
    grapple::{
        ...
        errors::GrappleError,
        lasercan::{
            LaserCanMeasurement, LaserCanMessage, LaserCanRangingMode, LaserCanRoi,
            LaserCanTimingBudget,
        },
        GrappleDeviceMessage, Request, TaggedGrappleMessage,
    },
    request_factory, DEVICE_ID_BROADCAST,
};
use grapple_hook_macros::rpc;
use tokio::sync::RwLock;

use super::{
    check_for_new_firmware_release_rpc_target, start_field_upgrade, Device,
    FirmwareValidatingDevice, GrappleDevice, GrappleDeviceRequest, GrappleDeviceResponse,
    HasFirmwareUpdateURLDevice, RootDevice, SendWrapper, SharedInfo, VersionGatedDevice,
};
use crate::{
    rpc::RpcBase,
    updates::{most_recent_update_available, LightReleaseResponse},
};
...
impl VersionGatedDevice for LaserCan {
    ...
    async fn check_for_new_firmware_release(current_version: &str) -> Option<LightReleaseResponse> {
        ...
        most_recent_update_available("https://github.com/GrappleRobotics/LaserCAN", |release| {
...
impl FirmwareValidatingDevice for LaserCan {
    fn validate_firmware(_info: &super::DeviceInfo, buf: &[u8]) -> anyhow::Result<()> {
        if &buf[0x150..0x154] == &[0xBEu8, 0xBAu8, 0xFEu8, 0xCAu8]
            && buf[0x15c] == (GrappleModelId::LaserCan as u8)
        {
            Ok(())
        } else {
...
impl LaserCan {
    ...
    async fn set_range(&self, mode: LaserCanRangingMode) -> anyhow::Result<()> {
        ...
        let (encode, decode) = request_factory!(
            data,
            GrappleDeviceMessage::DistanceSensor(LaserCanMessage::SetRange(data))
        );

        let msg = self
            .sender
            .request(TaggedGrappleMessage::new(id, encode(mode)), 300, 5)
            .await?;
        decode(msg.msg)??;
        Ok(())
    }
    ...
    async fn set_roi(&self, roi: LaserCanRoi) -> anyhow::Result<()> {
        ...
        let (encode, decode) = request_factory!(
            data,
            GrappleDeviceMessage::DistanceSensor(LaserCanMessage::SetRoi(data))
        );

        let msg = self
            .sender
            .request(TaggedGrappleMessage::new(id, encode(roi)), 300, 5)
            .await?;
        decode(msg.msg)??;
        Ok(())
    }
    ...
    async fn set_timing_budget(&self, budget: LaserCanTimingBudget) -> anyhow::Result<()> {
        ...
        let (encode, decode) = request_factory!(
            data,
            GrappleDeviceMessage::DistanceSensor(LaserCanMessage::SetTimingBudget(data))
        );

        let msg = self
            .sender
            .request(TaggedGrappleMessage::new(id, encode(budget)), 300, 5)
            .await?;
        decode(msg.msg)??;
        Ok(())
    }
